...
class MarkdownExtra extends \Michelf\Markdown {
	...
	protected function _doAnchors_reference_callback($matches) {
		...
		// Check this first
		if ($whole_match == "[ShowAllChatColors]")
		{
			// Grab our json file
			$Json_File = "../lib/shared.json";
			// Read our json file
			$jsonInfo = json_decode( file_get_contents($Json_File), true );
			// Make sure it exist!
			if ( empty( $jsonInfo ) )
				return "{$Json_File} not found!";
			else
			{
				$result = '<table class="table table-bordered table-hover"><thead><tr><th>Color Code</th><th>Usage</th></tr></thead><tbody>';
				foreach ($jsonInfo['colors'] as $color_key => $value)
					$result .= '<tr><td style="color: #'. $value .';">' . $color_key . '</td><td>{' . $color_key . '}</td></tr>';
				$result .= '</tbody></table>';
